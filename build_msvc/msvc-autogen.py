...
def parse_config_into_hrs_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Horus Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/horus_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/horus_config.h'), "w", encoding="utf8") as hrs_config:
        hrs_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Horus-core msbuild configuration initialiser.')
    ...
    parse_config_into_hrs_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/horus_config.h'), os.path.join(SOURCE_DIR, 'config/horus-config.h'))
