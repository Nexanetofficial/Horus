# User-space, Statically Defined Tracing (USDT) for Horus Core

Horus Core includes statically defined tracepoints to allow for more
...
**Note**: Horus Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Horus Core
...
Multiple tools can list the available tracepoints in a `horusd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Horus Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/horusd
stap net        outbound_message 0x0000000000107c05 /src/horusd
stap validation block_connected  0x00000000002fb10c /src/horusd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Horus Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/horusd | grep NT_STAPSDT -A 4 -B 2
