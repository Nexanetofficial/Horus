# PSBT Howto for Horus Core

Since Horus Core 0.17, an RPC interface exists for Partially Signed Horus
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/horus/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Horus transactions that are not fully signed
...
#### Multisig with multiple Horus Core instances
...
Horus Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `horus-cli` is used.
...
Later, when *V* HRS has been received on *Amulti*, and Bob and Carol want to
