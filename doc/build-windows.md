...
Below are some notes on how to build Horus Core for Windows.

The options known to work for building Horus Core on Windows are:
...
Note that for WSL the Horus Core source path MUST be somewhere in the default mount file system, for
example /usr/src/horus, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\horus`, for example:

    make install DESTDIR=/mnt/c/workspace/horus
