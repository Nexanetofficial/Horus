...
The Horus Core daemon can be configured to act as a trusted "border
router", implementing the horus wire protocol and relay, making
...
The ZeroMQ feature in Horus Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building horusd:
...
For instance:
    ...
    $ horusd -zmqpubhashtx=tcp://127.0.0.1:215767 \
               -zmqpubhashtx=tcp://192.168.1.2:215767 \
               -zmqpubhashblock="tcp://[::1]:215768" \
               -zmqpubrawtx=ipc:///tmp/horusd.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in horus.conf.
...
From the perspective of horusd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into horusd directly. Furthermore, no information is
...
using. Horusd appends an up-counting sequence number to each
