# CJDNS support in Horus Core

It is possible to run Horus Core over CJDNS, an encrypted IPv6 network that
...
done outside of Horus Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Horus network and individual nodes.
...
before it will work with your Horus Core node. This is described in steps
...
## Run Horus Core with CJDNS

Once you are connected to the CJDNS network, the following Horus Core
...
When enabled, this option tells Horus Core that it is running in an
...
IPv6 local network. This helps Horus Core perform better address management:
...
CJDNS support was added to Horus Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `horus-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/horus/horus/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Horus Core

There are several ways to see your CJDNS address in Horus Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `horus-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `horus-cli getpeerinfo`).
