...
TMPDIR_PREFIX = "horus_func_test_"
...
class HorusTestMetaClass(type):
    """Metaclass for HorusTestFramework.

    Ensures that any attempt to register a subclass of `HorusTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'HorusTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("HorusTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("HorusTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class HorusTestFramework(metaclass=HorusTestMetaClass):
    """Base class for a horus test script.

    Individual horus test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave horusds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop horusds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use horus-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_horusd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "horusd" + config["environment"]["EXEEXT"],
        )
        fname_horuscli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "horus-cli" + config["environment"]["EXEEXT"],
        )
        fname_horusutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "horus-util" + config["environment"]["EXEEXT"],
        )
        self.options.horusd = os.getenv("HORUSD", default=fname_horusd)
        self.options.horuscli = os.getenv("HORUSCLI", default=fname_horuscli)
        self.options.horusutil = os.getenv("HORUSUTIL", default=fname_horusutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: horusds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("HorusRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'horusd', self.options.horusd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'horus-cli', self.options.horuscli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a horusd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple horusd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as horusd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    horusd=self.options.horusd,
                    horus_cli=self.options.horuscli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in horus.conf
    ...
    def skip_if_no_horusd_tracepoints(self):
        """Skip the running test if horusd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("horusd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_horusd_zmq(self):
        """Skip the running test if horusd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("horusd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if horus-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("horus-wallet has not been compiled")

    def skip_if_no_horus_util(self):
        """Skip the running test if horus-util has not been compiled."""
        if not self.is_horus_util_compiled():
            raise SkipTest("horus-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if horus-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("horus-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether horus-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether horus-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_horus_util_compiled(self):
        """Checks whether horus-util was compiled."""
        return self.config["components"].getboolean("ENABLE_HORUS_UTIL")
