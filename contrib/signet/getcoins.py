...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing horus-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='horus-cli', help='horus-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Horus address to which the faucet should send')
...
parser.add_argument('horus_cli_args', nargs='*', help='Arguments to pass on to horus-cli (default: -signet)')

args = parser.parse_args()

if args.horus_cli_args == []:
    args.horus_cli_args = ['-signet']


def horus_cli(rpc_command_and_params):
    argv = [args.cmd] + args.horus_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = horus_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = horus_cli(['getnewaddress', 'faucet', 'bech32'])
