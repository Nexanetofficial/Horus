...
""" Example logging Horus Core mempool events using the mempool:added,
...
def main(horusd_path):
    horusd_with_usdts = USDT(path=str(horusd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    horusd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    horusd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    horusd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    horusd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[horusd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/horusd")
        exit(1)
