...
/** Call node.updateRwSetting() with Horus 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Horus 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/horus/horus/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because horus 22.x will not try to read these.
...
//! Convert enabled/size values to horus -prune setting.
...
//! Get pruning enabled value to show in GUI from horus -prune setting.
...
//! Get pruning size value to show in GUI from horus -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayHorusUnit")) {
        settings.setValue("DisplayHorusUnit", QVariant::fromValue(HorusUnit::HRS));
    }
    QVariant unit = settings.value("DisplayHorusUnit");
    if (unit.canConvert<HorusUnit>()) {
        m_display_horus_unit = unit.value<HorusUnit>();
    } else {
        m_display_horus_unit = HorusUnit::HRS;
        settings.setValue("DisplayHorusUnit", QVariant::fromValue(m_display_horus_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if horus.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_horus_unit);
...
{
    if (new_unit.isNull() || new_unit.value<HorusUnit>() == m_display_horus_unit) return;
    m_display_horus_unit = new_unit.value<HorusUnit>();
    QSettings settings;
    settings.setValue("DisplayHorusUnit", QVariant::fromValue(m_display_horus_unit));
    Q_EMIT displayUnitChanged(m_display_horus_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/horus/horus/pull/8273
    ...
    // (https://github.com/horus-core/gui/issues/567).
