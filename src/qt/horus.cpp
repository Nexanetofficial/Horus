...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<HorusUnit>("HorusUnit");
#else
    qRegisterMetaType<HorusUnit>("HorusUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. horus_de.qm (shortcut "de" needs to be defined in horus.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. horus_de_DE.qm (shortcut "de_DE" needs to be defined in horus.qrc)
...
static const char* qt_argv = "horus-qt";

HorusApplication::HorusApplication()
...
void HorusApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the HorusApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", HorusGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void HorusApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool HorusApplication::createOptionsModel(bool resetSettings)
{
...
void HorusApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new HorusGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &HorusGUI::quitRequested, this, &HorusApplication::requestShutdown);
...
void HorusApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void HorusApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool HorusApplication::baseInitialize()
{
    return node().baseInitialize();
}

void HorusApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &HorusApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &HorusApplication::handleRunawayException);
    connect(this, &HorusApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &HorusApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void HorusApplication::parameterSetup()
{
...
void HorusApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void HorusApplication::requestInitialize()
{
...
void HorusApplication::requestShutdown()
{
...
void HorusApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // horus: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &HorusGUI::handlePaymentRequest);
            connect(window, &HorusGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void HorusApplication::handleRunawayException(const QString &message)
{
...
void HorusApplication::handleNonFatalException(const QString& message)
{
...
WId HorusApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool HorusApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", HorusGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(horus);
    Q_INIT_RESOURCE(horus_locale);
    ...
    /// 6-7. Parse horus.conf, determine network, switch to network specific
    ...
    // horus: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
