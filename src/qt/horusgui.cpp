...
const std::string HorusGUI::DEFAULT_UIPLATFORM =
...
HorusGUI::HorusGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &HorusGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &HorusGUI::showModalOverlay);
...
void HorusGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Horus address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and horus: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &HorusGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &HorusGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &HorusGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Horus addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Horus addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Horus Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Horus Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a horus: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Horus command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &HorusGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &HorusGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &HorusGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &HorusGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &HorusGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &HorusGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &HorusGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &HorusGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &HorusGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &HorusGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &HorusGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &HorusGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &HorusGUI::showDebugWindow);
}

void HorusGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void HorusGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &HorusGUI::setCurrentWalletBySelectorIndex);
...
void HorusGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &HorusGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &HorusGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &HorusGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &HorusGUI::showProgress);
...
#ifdef ENABLE_WALLET
void HorusGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void HorusGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &HorusGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &HorusGUI::removeWallet);
...
WalletController* HorusGUI::getWalletController()
{
    return m_wallet_controller;
}

void HorusGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &HorusGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &HorusGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &HorusGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &HorusGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &HorusGUI::incomingTransaction);
    connect(this, &HorusGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void HorusGUI::removeWallet(WalletModel* walletModel)
{
...
void HorusGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void HorusGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void HorusGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void HorusGUI::setWalletActionsEnabled(bool enabled)
{
...
void HorusGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &HorusGUI::toggleHidden);
...
void HorusGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void HorusGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void HorusGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void HorusGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void HorusGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void HorusGUI::openClicked()
{
...
void HorusGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void HorusGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void HorusGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void HorusGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void HorusGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void HorusGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void HorusGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void HorusGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Horus network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void HorusGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void HorusGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void HorusGUI::updateHeadersSyncProgressLabel()
{
...
void HorusGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void HorusGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &HorusGUI::quitRequested);
...
void HorusGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void HorusGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void HorusGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &HorusGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &HorusGUI::show);
                e->ignore();
            }
        }
    }
...
void HorusGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void HorusGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void HorusGUI::incomingTransaction(const QString& date, HorusUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(HorusUnits::formatWithUnit(unit, amount, true));
...
void HorusGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void HorusGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool HorusGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool HorusGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void HorusGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void HorusGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void HorusGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void HorusGUI::updateWindowTitle()
{
...
void HorusGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void HorusGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void HorusGUI::detectShutdown()
{
...
void HorusGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void HorusGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(HorusGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = HorusGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void HorusGUI::subscribeToCoreSignals()
{
...
void HorusGUI::unsubscribeFromCoreSignals()
{
...
bool HorusGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<HorusUnit> units = HorusUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const HorusUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, HorusUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const HorusUnit u : HorusUnits::availableUnits()) {
        menu->addAction(HorusUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(HorusUnit newUnits)
{
    setText(HorusUnits::longName(newUnits));
}
