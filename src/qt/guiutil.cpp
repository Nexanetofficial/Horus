...
#include <qt/horusaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Horus address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new HorusAddressEntryValidator(parent));
    widget->setCheckValidator(new HorusAddressCheckValidator(parent));
}
...
bool parseHorusURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no horus: URI
    if(!uri.isValid() || uri.scheme() != QString("horus"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!HorusUnits::parse(HorusUnit::HRS, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseHorusURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseHorusURI(uriInstance, out);
}

QString formatHorusURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("horus:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(HorusUnits::format(HorusUnit::HRS, info.amount, false, HorusUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openHorusConf()
{
    ...
    /* Open horus.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Horus.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Horus (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Horus (%s).lnk", chain));
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "horus.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("horus-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a horus.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Horus\n";
        else
            optionFile << strprintf("Name=Horus (%s)\n", chain);
