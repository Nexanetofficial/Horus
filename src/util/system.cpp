...
const char * const HORUS_CONF_FILENAME = "horus.conf";
const char * const HORUS_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //horus-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", HORUS_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Horus
    // macOS: ~/Library/Application Support/Horus
    // Unix-like: ~/.horus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Horus";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Horus";
#else
    // Unix-like
    return pathRet / ".horus";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", HORUS_CONF_FILENAME));
}
